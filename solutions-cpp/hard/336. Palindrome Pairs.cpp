    ...
    vector<vector<int>> palindromePairs(vector<string> &A) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            for (int j = 0; j < s.size() && node; ++j) {
                ...
                // A[i]'s prefix matches this word and A[i]'s suffix forms a
                // palindrome
            ...
            for (int j : node->palindromeIndexes) {
                ...
                // If a word whose prefix is palindrome after matching its
                // suffix with A[i], then this is also a valid pair
